...
plugins.withType<KotlinBasePluginWrapper>().configureEach {
    extensions.configure<KotlinProjectExtension>("kotlin") {
        ...
        // true by default
        val warningsAsErrors = providers.gradleProperty("clozybuild.warningsAsErrors").orNull?.toBoolean() ?: true

        if (providers.gradleProperty("clozybuild.skipTests").map(String::toBoolean).getOrElse(false)) {
